...
`@MainActor @Observable` orchestrator; owns `SYSTEM_PROMPT_TEMPLATE`, conversation history, tool registry, active backend. The system prompt is a **per-session snapshot** built from: snapshot time, `memory.contextSummary`, PMC/load/ATP sections (`ProactiveCoach.promptSection`/`loadPromptSection`, `ATPToolHandler.promptSection`), and a data/devices section listing active sources + target. `setSources(read:write:)` rebuilds the registry via `configureTools`.

The snapshot (`sessionSystemPrompt`) is frozen for both backends — byte-identical requests keep the provider's prompt cache hitting, and it's what the Apple FM session is built with. It's rebuilt only where the Apple session is (`reseedBackend` on backend/source/settings change, `reset`); data changes mid-session are deliberately *not* patched in — the prompt tells the model the snapshot time and to use tools for anything newer. The clock reaches the model as a `[Current time: …]` prefix on the user message once 30+ min have passed since the last time it was told (snapshot or previous stamp); `rewind` clears that marker so the next message is stamped. Request JSON is serialized with `.sortedKeys` so tool schemas and replayed tool-call arguments are byte-stable across launches.
...
- **`AppleFoundationModelBackend`** (`AppleFoundationModelService.swift`) — on-device, self-managing via a persistent `LanguageModelSession`. `CoachToolBridge.swift` converts each tool's JSON-Schema `parameters` into a FoundationModels `GenerationSchema` at runtime, so tools are defined once for both backends.
