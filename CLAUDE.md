...
- Single Xcode project, single target/scheme (`TriGenius`), **no test target**. Requires **Xcode-beta** (deployment targets iOS 27+/macOS 27+; the Apple Intelligence backend needs the iOS/macOS 27 FoundationModels framework). Prefer building/running through the IDE.
- Xcode **synchronized groups** (`PBXFileSystemSynchronizedRootGroup`, `objectVersion = 90`): the `TriGenius/` tree maps directly to the target — new `.swift` files anywhere under it are picked up automatically; **do not** edit `project.pbxproj`.
...
- **`coach_memory.json`** (Application Support) — the coach's *prompt context*: profile, preferences, weekly structure, plan, feedback. Deliberately separate from the time-series store.
...
- Reference only — **`ref/garmin_health_data/`**: a vendored Python Garmin Connect client (not in the build), kept as a worked example of endpoint URLs, request/response shapes, and the token exchange. Start with `garmin_client/api.py`, `constants.py`, `client.py`. **Do not import or ship it** — port behavior into the Swift layer.
...
- **`Analytics/`** — a pure, store-fed layer (no networking): `PMCEngine` (CTL/ATL/TSB from daily TSS, TrainingPeaks EWMA), `TSS*` (scoring/calculation/constants), `HRZones` (HR-zone bounds from LTHR/max HR + bucket a HR stream into time-in-zone), `TrainingLoadAnalytics`, `WeeklyTarget`, `DashboardInsight`. Mostly `enum`/`struct` `static` functions over `WorkoutRecord` arrays — no shared state.
...
**Memory** (`Coach/CoachMemory.swift`) — `ObservableObject` persisting profile, preferences, weekly structure, plan, sport progress, feedback as `coach_memory.json` (Application Support); snake_case keys, each model has `init(from:)`/`toDict()`. `contextSummary` is injected into the system prompt; "FEHLENDE INFORMATIONEN" drives the onboarding flow.
